...
function Player({src,active,id}:{src:string;active:boolean;id:string}) {
  const items = useMemo(() => [{id,title:id,kind:'video' as const,detail:'Mock media only',free:true,ownedOrConsumed:false,premiumRequired:false,mediaUrl:src}], [id,src]);
  return <ImmersiveMediaFeed items={items} empty="Mock media unavailable" mediaPaused={!active} />;
}
function Fixture() {
  ...
  useEffect(() => {
    const canvas = root.current?.querySelector<HTMLElement>('[data-canvas]');
    if (canvas) return attachNavDockDoubleTap(canvas, () => runtime.dispatch({type:'DOUBLE_TAP_CANVAS'}));
  }, [runtime.dispatch]);
  ...
  return <div ref={root} data-fixture="space-contract-v1">
    <header><strong>TEST FIXTURE — LifeOS</strong> <span>{status}</span>
