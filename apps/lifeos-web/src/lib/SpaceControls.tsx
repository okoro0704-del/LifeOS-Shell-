...
import type { SpaceDefinition, SpaceEvent, SpaceRuntime } from './space-runtime';

export function SpaceControls({ state, definition, dispatch, children, interactionsAvailable = true }: {
  state: SpaceRuntime; definition: SpaceDefinition; dispatch: (event: SpaceEvent) => void; children?: ReactNode; interactionsAvailable?: boolean;
}) {
  ...
  return <div data-space-controls={state.presentationState} style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 90 }}>
    ...
    {controls ? <nav aria-label="Space controls" style={{ position: 'absolute', right: 24, bottom: 24,
      ...
      <button type="button" disabled={!interactionsAvailable} title={interactionsAvailable ? undefined : 'Space interaction adapter is not implemented'} onClick={() => dispatch({ type: 'OPEN_INTERACTIONS' })}>Interactions</button>
